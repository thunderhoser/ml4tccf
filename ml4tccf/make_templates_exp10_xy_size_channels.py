"""Makes templates for Experiment 10."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment10_xy_size_channels/templates'
)
...
def _run():
    ...
    for i in range(len(GRID_ROW_COUNTS)):
        for j in range(len(LAG_TIME_COUNTS)):
            for k in range(len(FIRST_LAYER_FILTER_COUNTS)):
                option_dict = copy.deepcopy(DEFAULT_OPTION_DICT)
                input_dimensions = numpy.array([
                    GRID_ROW_COUNTS[i], GRID_ROW_COUNTS[i],
                    LAG_TIME_COUNTS[j], 5
                ], dtype=int)
                ...
                if GRID_ROW_COUNTS[i] == 300:
                    num_pixels_coarsest = 16
                elif GRID_ROW_COUNTS[i] == 400:
                    num_pixels_coarsest = 36
                elif GRID_ROW_COUNTS[i] == 500:
