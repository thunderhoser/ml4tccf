...
def _run(archer_file_pattern, raw_best_track_dir_name, geocenter_dir_name,
    ...
    geocenter_file_pattern = '/mnt/shnas10/users/lagerquist/ml4tccf_project/geocenter_models/ensemble/real_time_predictions_short_track/isotonic_regression_gaussian_dist/uncertainty_calibration/for_zhixing/*/*.txt'

    geocenter_file_names = glob.glob(geocenter_file_pattern)
    geocenter_file_names.sort()

    num_storm_objects = len(geocenter_file_names)
    ...
    geocenter_valid_times_unix_sec = numpy.full(num_storm_objects, -1, dtype=int)
    geocenter_cyclone_id_strings = [''] * num_storm_objects

    for i in range(num_storm_objects):
        print('Reading data from: "{0:s}"...'.format(geocenter_file_names[i]))
        (
            geocenter_pred_latitudes_deg_n[i],
            geocenter_pred_longitudes_deg_e[i],
            geocenter_valid_times_unix_sec[i]
        ) = _read_geocenter_file(geocenter_file_names[i])

        this_pathless_file_name = os.path.split(geocenter_file_names[i])[-1]
        this_cyclone_id_string = this_pathless_file_name.split('_')[0]
        this_cyclone_id_string = '{0:s}{1:s}'.format(
            this_cyclone_id_string[4:], this_cyclone_id_string[:4].upper()
        )
        geocenter_cyclone_id_strings[i] = this_cyclone_id_string
    ...
    for i in range(num_storm_objects):
        if (
                i == 0 or
                geocenter_cyclone_id_strings[i] !=
                geocenter_cyclone_id_strings[i - 1]
        ):
            print(SEPARATOR_STRING)
            ...
            best_track_file_name = _find_best_track_file(
                ...
                cyclone_id_string=geocenter_cyclone_id_strings[i]
            )
        ...
        (
            ...
            geocenter_target_longitudes_deg_e[i]
                ...
                valid_time_unix_sec=geocenter_valid_times_unix_sec[i],
    ...
    good_indices = numpy.where(numpy.invert(numpy.logical_or(
        numpy.isnan(geocenter_target_latitudes_deg_n),
        numpy.isnan(geocenter_target_longitudes_deg_e)
    )))[0]
    num_storm_objects = len(good_indices)
