"""Plots predicted TC centers on top of satellite images for 2025 WPO prpsl."""
...
BEST_TRACK_DIR_ARG_NAME = 'input_best_track_dir_name'
...
BEST_TRACK_DIR_HELP_STRING = (
    'Path to directory with best-track files.  Files therein will be found by '
    '`_find_best_track_file`, and read by `_read_best_track_file`, both '
...
def _find_best_track_file(directory_name, cyclone_id_string,
    ...
    :param target_time_unix_sec: Target time, i.e., time at which best-track
        center is desired.
    ...
    pickle_file_pattern = '{0:s}/storm_track_interp_b{1:s}_*.pkl'.format(
...
def _read_best_track_file(pickle_file_name, target_time_unix_sec):
...
def _make_figure_one_example(
        ...
        best_track_latitude_deg_n, best_track_longitude_deg_e,
        prediction_matrix, model_metadata_dict,
    ...
    :param best_track_latitude_deg_n: Latitude (deg north) of best-track center.
    :param best_track_longitude_deg_e: Longitude (deg east) of best-track
        center.
    ...
    best_track_longitude_deg_e = lng_conversion.convert_lng_negative_in_west(
        best_track_longitude_deg_e
    )
    ...
    y_error_km = numpy.nan
    x_error_km = numpy.nan
    euclidean_error_km = numpy.nan
    ...
    if prediction_plotting_format_string == PROB_CONTOURS_FORMAT_STRING:
        ...
        first_point = (
            numpy.mean(ensemble_latitudes_deg_n),
            numpy.mean(ensemble_longitudes_deg_e)
        )
        second_point = (
            best_track_latitude_deg_n, numpy.mean(ensemble_longitudes_deg_e)
        )
        y_error_km = METRES_TO_KM * geodesic(first_point, second_point).meters

        midpoint_latitude_deg_n = numpy.mean(numpy.array([
            numpy.mean(ensemble_latitudes_deg_n),
            best_track_latitude_deg_n
        ]))
        first_point = (
            midpoint_latitude_deg_n,
            numpy.mean(ensemble_longitudes_deg_e)
        )
        second_point = (
            midpoint_latitude_deg_n, best_track_longitude_deg_e
        )
        x_error_km = METRES_TO_KM * geodesic(first_point, second_point).meters
    ...
    for i in range(num_lag_times):
        for j in range(num_wavelengths):
            ...
            axes_object.plot(
                best_track_longitude_deg_e, best_track_latitude_deg_n,
...
def _run(prediction_file_name, satellite_dir_name, normalization_file_name,
    ...
    :param best_track_dir_name: Same.
    ...
    if data_type_string == nn_utils.CIRA_IR_DATA_TYPE_STRING:
        data_dict = nn_training_cira_ir.create_data_specific_trans(
            ...
            row_translations_low_res_px=numpy.full(num_times, 0, dtype=int),
            column_translations_low_res_px=numpy.full(num_times, 0, dtype=int)
        )
    ...
    for i in range(num_examples):
        ...
        best_track_file_name = _find_best_track_file(
            directory_name=best_track_dir_name,
            cyclone_id_string=cyclone_id_string,
            target_time_unix_sec=target_times_unix_sec[i]
        )
        assert best_track_file_name is not None
        ...
        assert best_track_latitude_deg_n is not None

        _make_figure_one_example(
            brightness_temp_matrix_kelvins=
            brightness_temp_matrix_kelvins[i, ...],
            best_track_latitude_deg_n=best_track_latitude_deg_n,
            best_track_longitude_deg_e=best_track_longitude_deg_e,
...
if __name__ == '__main__':
    ...
    _run(
        ...
        best_track_dir_name=getattr(INPUT_ARG_OBJECT, BEST_TRACK_DIR_ARG_NAME),
