"""Makes templates for ensemble version of Experiment 11."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment11_xy_size_channels_ensemble/templates'
)
...
ENSEMBLE_MEMBERS_AXIS2 = numpy.linspace(1, 13, num=13, dtype=int)
...
def _run():
    ...
    for i in range(len(WAVELENGTH_GROUP_STRINGS_AXIS1)):
        for j in range(len(ENSEMBLE_MEMBERS_AXIS2)):
            ...
            output_file_name = (
                '{0:s}/wavelengths-microns={1:s}_ensemble-member={2:02d}/'
                'model.h5'
            ).format(
                OUTPUT_DIR_NAME,
                WAVELENGTH_GROUP_STRINGS_AXIS1[i],
                ENSEMBLE_MEMBERS_AXIS2[j]
            )
