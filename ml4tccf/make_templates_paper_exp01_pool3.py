"""Makes templates for Paper Experiment 1 (3-by-3 pooling)."""
...
OUTPUT_DIR_NAME = (
    ...
    'paper_experiment01_pool3/templates'
)
...
OPTIMIZER_FUNCTION = (
    accum_grad_optimizer.convert_to_accumulate_gradient_optimizer(
        ...
        update_params_frequency=7, accumulate_sum_or_mean=False
    )
)
...
LOSS_FUNCTION = custom_losses_scalar.coord_avg_crps_kilometres
...
GRID_ROW_COUNTS_AXIS1 = numpy.array([300, 300, 400, 500, 600, 700], dtype=int)
GRID_ROW_COUNTS_AXIS1_DUMMY = [
    '300-pool-by-2', '300', '400', '500', '600', '700'
]
MAIN_POOLING_SIZES_PX_AXIS1 = numpy.array([2, 3, 3, 3, 3, 3], dtype=int)
        ...
        for j in range(len(LAG_TIME_COUNTS_AXIS2)):
            for k in range(len(FIRST_LAYER_FILTER_COUNTS_AXIS3)):
                ...
                if GRID_ROW_COUNTS_AXIS1[i] == 300:
                    ...
                    num_pixels_coarsest = 16
                elif GRID_ROW_COUNTS_AXIS1[i] == 600:
                    pooling_size_by_conv_block_px = numpy.array(
                        [2, 3, 3, 3, 2, 2], dtype=int
                    )
                    num_channels_multipliers = numpy.array([
