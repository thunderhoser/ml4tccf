"""Makes templates for Paper Experiment 2 with short-track first guess."""
...
OUTPUT_DIR_NAME = (
    ...
    'paper_experiment02_with_short_track/templates'
)
...
DEFAULT_OPTION_DICT = {
    ...
    tcnn_architecture.OPTIMIZER_FUNCTION_KEY: keras.optimizers.AdamW()
}

GRID_ROW_COUNTS_AXIS1 = numpy.array([300, 400, 500, 600], dtype=int)
MAIN_POOLING_FACTORS_AXIS2 = numpy.array([2, 3], dtype=int)

# TODO(thunderhoser): Will probably make this 1, 3, 5, 7, 9, 11, 13.
LAG_TIME_COUNTS_AXIS3 = numpy.array([1, 2, 3, 4, 5, 6, 7], dtype=int)
...
def _run():
    ...
    for i in range(len(GRID_ROW_COUNTS_AXIS1)):
        for j in range(len(MAIN_POOLING_FACTORS_AXIS2)):
            for k in range(len(LAG_TIME_COUNTS_AXIS3)):
                option_dict = copy.deepcopy(DEFAULT_OPTION_DICT)

                input_dimensions = numpy.array([
                    GRID_ROW_COUNTS_AXIS1[i], GRID_ROW_COUNTS_AXIS1[i],
                    LAG_TIME_COUNTS_AXIS3[k], 3
                ], dtype=int)

                if GRID_ROW_COUNTS_AXIS1[i] == 300:
                    if MAIN_POOLING_FACTORS_AXIS2[j] == 2:
                ...
                elif GRID_ROW_COUNTS_AXIS1[i] == 500:
                    if MAIN_POOLING_FACTORS_AXIS2[j] == 2:
                        pooling_size_by_conv_block_px = numpy.full(
                            7, 2, dtype=int
                        )
                ...
                output_file_name = (
                    ...
                    MAIN_POOLING_FACTORS_AXIS2[j],
                    LAG_TIME_COUNTS_AXIS3[k]
                )
