"""Makes temporal-CNN templates for Structure Experiment 3-R34."""
...
OUTPUT_DIR_NAME = (
    ...
    'structure_experiment03_r34/templates'
)

NUM_SCALAR_PREDICTORS = 8
ENSEMBLE_SIZE = 50
NUM_DENSE_LAYERS = 4
...
METRIC_FUNCTION_STRINGS = [
    'custom_metrics_structure.mean_squared_error(channel_index=0, target_shrink_factor={0:.6f}, function_name="mse_r34_km2")'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.min_target(channel_index=0, target_shrink_factor={0:.6f}, function_name="min_target_r34_km")'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.max_target(channel_index=0, target_shrink_factor={0:.6f}, function_name="max_target_r34_km")'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.min_prediction(channel_index=0, target_shrink_factor={0:.6f}, function_name="min_single_pred_r34_km", take_ensemble_mean=False)'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.max_prediction(channel_index=0, target_shrink_factor={0:.6f}, function_name="max_single_pred_r34_km", take_ensemble_mean=False)'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.min_prediction(channel_index=0, target_shrink_factor={0:.6f}, function_name="min_avg_pred_r34_km", take_ensemble_mean=True)'.format(TARGET_SHRINK_FACTOR),
    'custom_metrics_structure.max_prediction(channel_index=0, target_shrink_factor={0:.6f}, function_name="max_avg_pred_r34_km", take_ensemble_mean=True)'.format(TARGET_SHRINK_FACTOR),
]
...
DEFAULT_OPTION_DICT = {
    ...
    tcnn_architecture.INTENSITY_INDEX_KEY: 1,
    tcnn_architecture.R34_INDEX_KEY: 0,
...
USE_PHYS_CONSTRAINT_FLAGS_AXIS2 = numpy.array([0], dtype=bool)
...
def _run():
    ...
    dense_neuron_counts = architecture_utils.get_dense_layer_dimensions(
        num_input_units=num_pixels_coarsest * num_channels_by_conv_layer[-1],
        num_classes=1,
