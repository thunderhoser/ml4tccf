...
MODEL_DIR_NAMES=("/mnt/shnas10/users/lagerquist/ml4tccf_project/geocenter_models/wavelengths-microns=3.900-7.340-13.300")
...
for model_dir_name in "${MODEL_DIR_NAMES[@]}"; do
    ...
    for mean_translation_distance_px in "${MEAN_TRANSLATION_DISTANCES_PX[@]}"; do
        log_file_name="apply_iso_reg_second_model_mean-trans-dist-px=${mean_translation_distance_px}.out"
    
        python3 -u "${CODE_DIR_NAME}/apply_scalar_iso_regression.py" &> ${log_file_name} \
        --input_prediction_file_name="${model_dir_name}/sensitivity_analysis_to_trans_dist/mean_translation_distance_px=${mean_translation_distance_px}/predictions_${CYCLONE_ID_STRING}_${VALID_DATE_STRING}.nc" \
        --input_model_file_name="${model_dir_name}/isotonic_regression/isotonic_regression.dill" \
        --output_prediction_file_name="${model_dir_name}/sensitivity_analysis_to_trans_dist/mean_translation_distance_px=${mean_translation_distance_px}/isotonic_regression/predictions_${CYCLONE_ID_STRING}_${VALID_DATE_STRING}.nc" \
