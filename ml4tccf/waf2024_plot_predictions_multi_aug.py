...
For this script, each figure is a multi-panel with one translation vector
(i.e., one random data augmentation) per panel.
"""
...
WAVELENGTH_ARG_NAME = 'wavelength_microns'
...
WAVELENGTH_HELP_STRING = (
    'Wavelength at which to plot predictor (satellite) data.'
)
...
def _plot_prob_contours_1panel(
    ...
    axes_object.contour(
        ...
        linewidths=3.25, linestyles='solid', zorder=1e12
    )
...
def _plot_point_predictions_1panel(
    ...
    for k in range(ensemble_size):
        axes_object.plot(
            ...
            markersize=PREDICTED_CENTER_MARKER_SIZE_MULT * marker_size,
...
def _make_figure_one_example(
        brightness_temp_matrix_kelvins, target_matrix, prediction_matrix,
    ...
    D = number of data augmentations (random translations)
    M = number of rows in grid
    N = number of columns in grid
    ...
    :param brightness_temp_matrix_kelvins: D-by-M-by-N-by-L-by-W numpy array of
        brightness temperatures.
    :param target_matrix: D-by-4 numpy array with the following column order:
        [true row offset, true column offset, grid spacing in km,
        true TC-center latitude in deg N].
    :param prediction_matrix: D-by-2-by-S numpy array, where
        prediction_matrix[:, 0, :] contains predicted row offsets and
        prediction_matrix[:, 1, :] contains predicted column offsets.
    :param btemp_latitude_matrix_deg_n: D-by-M-by-L numpy array of latitudes
        (deg north).
    :param btemp_longitude_matrix_deg_e: D-by-N-by-L numpy array of longitudes
        (deg east).
    :param border_latitudes_deg_n: length-P numpy array of latitudes (deg
        north).
    :param border_longitudes_deg_e: length-P numpy array of longitudes (deg
    ...
    num_augmentations = brightness_temp_matrix_kelvins.shape[0]
    ...
    for d in range(num_augmentations):
        this_btemp_matrix_kelvins = brightness_temp_matrix_kelvins[d, ...]
        these_target_values = target_matrix[d, :]
        this_prediction_matrix = prediction_matrix[d, ...]
        this_latitude_matrix_deg_n = btemp_latitude_matrix_deg_n[d, ...]

        this_longitude_matrix_deg_e = (
            lng_conversion.convert_lng_negative_in_west(
                btemp_longitude_matrix_deg_e[d, ...]
            )
        )
        longitude_range_deg = (
        ...
        actual_x_center_transaxes = (
            0.5 + these_target_values[1] / num_grid_columns
        )
        ...
        x_error_km = grid_spacing_km * (
            numpy.mean(this_prediction_matrix[1, :]) - these_target_values[1]
        )
        euclidean_error_km = numpy.sqrt(y_error_km ** 2 + x_error_km ** 2)

        y_stdev_km = grid_spacing_km * numpy.std(
            this_prediction_matrix[0, :], ddof=1
        )
        x_stdev_km = grid_spacing_km * numpy.std(
            this_prediction_matrix[1, :], ddof=1
        )
        euclidean_stdev_km = ss_utils._get_predictive_stdistdevs(
            grid_spacing_km * numpy.expand_dims(this_prediction_matrix, axis=0)
        )[0]

        if prediction_plotting_format_string == MEAN_POINT_FORMAT_STRING:
            this_prediction_matrix = numpy.mean(
                this_prediction_matrix, axis=-1, keepdims=True
            )

        # Convert predictions to whatever format is needed for plotting.
        ensemble_size = this_prediction_matrix.shape[1]

        if prediction_plotting_format_string == PROB_CONTOURS_FORMAT_STRING:

            # TODO(thunderhoser): Does not handle issues with Int'l Date Line.
            latitude_interp_object = interp1d(
                x=all_row_indices, y=this_latitude_matrix_deg_n[:, -1],
                kind='linear', bounds_error=True
            )
            longitude_interp_object = interp1d(
                x=all_column_indices, y=this_longitude_matrix_deg_e[:, -1],
                kind='linear', bounds_error=True
            )

            ensemble_latitudes_deg_n = numpy.array([
                latitude_interp_object(
                    center_row_index + this_prediction_matrix[0, k]
                )
                for k in range(ensemble_size)
            ])
            ...
            ensemble_longitudes_deg_e = numpy.array([
                longitude_interp_object(
                    center_column_index + this_prediction_matrix[1, k]
                )
                for k in range(ensemble_size)
            ])
            ...
            prob_matrix = misc_utils.latlng_points_to_probability_grid(
                ...
                grid_latitude_array_deg_n=this_latitude_matrix_deg_n[:, -1],
                grid_longitude_array_deg_e=this_longitude_matrix_deg_e[:, -1]
            )
            ...
            if numpy.any(prob_matrix > TOLERANCE):
                min_colour_value = numpy.min(
                    prob_matrix[prob_matrix > TOLERANCE]
                )
            else:
                min_colour_value = 0.
        ...
        this_file_name = '{0:s}_{1:d}th.jpg'.format(
        ...
        figure_object, axes_object = _plot_predictors_1panel(
            ...
            this_btemp_matrix_kelvins[..., 0, 0],
            grid_latitudes_deg_n=this_latitude_matrix_deg_n[:, 0],
            grid_longitudes_deg_e=this_longitude_matrix_deg_e[:, 0],
            border_latitudes_deg_n=border_latitudes_deg_n,
            border_longitudes_deg_e=border_longitudes_deg_e,
    ...
    # TODO(thunderhoser): The colour scale is actually different for every
    # panel, but the exact probs don't really matter anyways.
    plotting_utils.add_colour_bar(
        figure_file_name=output_file_name,
        colour_map_object=prob_colour_map_object,
        colour_norm_object=prob_colour_norm_object,
...
def _run(prediction_file_name, satellite_dir_name, normalization_file_name,
         target_time_strings, wavelength_microns,
         prediction_plotting_format_string, prob_colour_map_name,
         prob_contour_smoothing_radius_px, point_prediction_marker_size,
    ...
    vod[nn_utils.LAG_TIMES_KEY] = numpy.array([0], dtype=int)
    vod[nn_utils.LOW_RES_WAVELENGTHS_KEY] = numpy.array(
        [wavelength_microns], dtype=float
    )
    ...
    if len(brightness_temp_matrix_kelvins.shape) != 5:
        brightness_temp_matrix_kelvins = (
            nn_utils.separate_lag_times_and_wavelengths(
                brightness_temp_matrix_kelvins, num_lag_times=1
            )
        )
    ...
    for unique_time_unix_sec in numpy.unique(target_times_unix_sec):
        same_time_indices = numpy.where(
            target_times_unix_sec == unique_time_unix_sec
        )[0]
        ...
        output_file_name = '{0:s}/{1:s}_{2:s}.jpg'.format(
            ...
            time_conversion.unix_sec_to_string(
                unique_time_unix_sec, TIME_FORMAT
            )
        )

        _make_figure_one_example(
            brightness_temp_matrix_kelvins=
            brightness_temp_matrix_kelvins[same_time_indices, ...],
            target_matrix=target_matrix[same_time_indices, ...],
            prediction_matrix=prediction_matrix[same_time_indices, ...],
            btemp_latitude_matrix_deg_n=
            btemp_latitude_matrix_deg_n[same_time_indices, ...],
            btemp_longitude_matrix_deg_e=
            btemp_longitude_matrix_deg_e[same_time_indices, ...],
            border_latitudes_deg_n=border_latitudes_deg_n,
            border_longitudes_deg_e=border_longitudes_deg_e,
...
if __name__ == '__main__':
    ...
    _run(
        ...
        wavelength_microns=getattr(INPUT_ARG_OBJECT, WAVELENGTH_ARG_NAME),
