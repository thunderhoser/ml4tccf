"""Makes templates for Experiment 6 -- 3 channels and 7 scalar variables."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment06_channels_and_scalars/templates'
)

NUM_SCALAR_PREDICTORS = 7
NUM_CONV_BLOCKS = 7
ENSEMBLE_SIZE = 5
NUM_DENSE_LAYERS = 4
...
DEFAULT_OPTION_DICT = {
    cnn_architecture.INPUT_DIMENSIONS_LOW_RES_KEY:
        numpy.array([580, 900, 15], dtype=int),
...
def _run():
    ...
    num_channels_by_conv_layer = (
        ...
        numpy.linspace(1, NUM_CONV_BLOCKS, num=NUM_CONV_BLOCKS, dtype=int)
    )

    dense_neuron_counts = (
        architecture_utils.get_dense_layer_dimensions(
            num_input_units=
            NUM_SCALAR_PREDICTORS + 4 * 7 * num_channels_by_conv_layer[-1],
            num_classes=2,
    ...
    if NUM_BATCHES_PER_UPDATE > 1:
        optimizer_function = (
            accum_grad_optimizer.convert_to_accumulate_gradient_optimizer(
                orig_optimizer=keras.optimizers.Adam(),
                update_params_frequency=NUM_BATCHES_PER_UPDATE,
    ...
    neural_net_utils.write_metafile(
        ...
        training_option_dict={neural_net_utils.SEMANTIC_SEG_FLAG_KEY: False},
        num_validation_batches_per_epoch=16,
        validation_option_dict={neural_net_utils.SEMANTIC_SEG_FLAG_KEY: False},
