"""Plots error metrics as a function of hyperparams for Experiment 11."""
...
GRID_ROW_COUNTS_AXIS1 = ['300-pool-by-2', '300', '400', '500', '590']
LAG_TIME_COUNTS_AXIS2 = numpy.array([3, 4, 5, 6, 7], dtype=int)
SPECTRAL_COMPLEXITIES_AXIS3 = numpy.array([10, 15, 20, 25], dtype=int)
...
def _run(experiment_dir_name):
    ...
    length_axis1 = len(GRID_ROW_COUNTS_AXIS1)
    length_axis2 = len(LAG_TIME_COUNTS_AXIS2)
    length_axis3 = len(SPECTRAL_COMPLEXITIES_AXIS3)
    num_metrics = len(METRIC_NAMES)

    y_tick_labels = [
        g.replace('-pool-by-2', '*') for g in GRID_ROW_COUNTS_AXIS1
    ]
    y_tick_labels = ['{0:s} x {0:s}'.format(y) for y in y_tick_labels]
    x_tick_labels = ['{0:d}'.format(l) for l in LAG_TIME_COUNTS_AXIS2]

    y_axis_label = 'Grid size'
