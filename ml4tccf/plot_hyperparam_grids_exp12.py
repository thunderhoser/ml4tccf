"""Plots error metrics as a function of hyperparams for Experiment 12."""
...
def _print_ranking_all_metrics(metric_matrix, main_metric_name):
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            'Num dense layers = {0:d} ... '
            'dense-layer dropout rate = {1:.3f} ... '
            'L_2 weight = 10^{2:.1f} ... '
            'Euc-distance-based ranks (mean, median, RMS) = '
            '{3:.1f}, {4:.1f}, {5:.1f} ... '
            'reliability rank = {6:.1f} ... CRPS rank = {7:.1f} ... '
...
def _print_ranking_one_metric(metric_matrix, metric_index):
    ...
    for m in range(len(i_sort_indices)):
        ...
        print((
            ...
            'num dense layers = {4:d} ... '
            'dense-layer dropout rate = {5:.3f} ... '
            'L_2 weight = 10^{6:.1f}'
        ).format(
...
def _run(experiment_dir_name):
    ...
    length_axis1 = len(DENSE_LAYER_COUNTS_AXIS1)
    length_axis2 = len(DENSE_LAYER_DROPOUT_RATES_AXIS2)
    length_axis3 = len(L2_WEIGHTS_AXIS3)
    num_metrics = len(METRIC_NAMES)

    y_tick_labels = ['{0:d}'.format(c) for c in DENSE_LAYER_COUNTS_AXIS1]
    x_tick_labels = [
        '{0:.3f}'.format(d) for d in DENSE_LAYER_DROPOUT_RATES_AXIS2
    ]

    y_axis_label = 'Number of dense layers'
    x_axis_label = 'Dropout rate for dense layers'
    ...
    for m in range(num_metrics):
        ...
        for k in range(length_axis3):
            ...
            panel_file_names[k] = '{0:s}/{1:s}_l2-weight={2:.10f}.jpg'.format(
                output_dir_name,
