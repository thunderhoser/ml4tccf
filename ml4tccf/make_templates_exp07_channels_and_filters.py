"""Makes templates for Experiment 7 -- tweak input channels and num filters."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment07_channels_and_filters/templates'
)
...
INPUT_WAVELENGTH_COUNTS = numpy.array([3, 10], dtype=int)
...
def _run():
    ...
    for i in range(len(FIRST_LAYER_FILTER_COUNTS)):
        for j in range(len(INPUT_WAVELENGTH_COUNTS)):
            for k in range(len(BATCHES_PER_UPDATE_COUNTS)):
                option_dict = copy.deepcopy(DEFAULT_OPTION_DICT)

                input_dimensions = numpy.array(
                    [580, 900, 5 * INPUT_WAVELENGTH_COUNTS[j]], dtype=int
                )
                ...
                output_file_name = (
                    ...
                    'num-input-wavelengths={2:02d}_'
                    'num-batches-per-update={3:d}/model.h5'
                ).format(
                    OUTPUT_DIR_NAME,
                    FIRST_LAYER_FILTER_COUNTS[i],
                    INPUT_WAVELENGTH_COUNTS[j],
                    BATCHES_PER_UPDATE_COUNTS[k]
                )
