"""Makes templates for Experiment 8 -- tweak dropout, train w/ all storms."""
...
OUTPUT_DIR_NAME = (
    ...
    'experiment08_dropout_all_storm_types/templates'
)

NUM_SCALAR_PREDICTORS = 16
NUM_CONV_BLOCKS = 7
ENSEMBLE_SIZE = 50
NUM_DENSE_LAYERS = 4
...
FIRST_LAYER_FILTER_COUNTS = numpy.array([4, 8, 12, 16], dtype=int)
DROPOUT_RATES = numpy.array([0.125, 0.25, 0.375, 0.5])
BATCHES_PER_UPDATE_COUNTS = numpy.array([2, 3], dtype=int)
        ...
        for j in range(len(DROPOUT_RATES)):
            for k in range(len(BATCHES_PER_UPDATE_COUNTS)):
                ...
                dense_dropout_rates = numpy.full(
                    NUM_DENSE_LAYERS, DROPOUT_RATES[j]
                )
