...
OUTPUT_DIR_NAME = (
    ...
    'paper_experiment05_domain_video_distribution/templates'
)
...
LAG_TIME_COUNTS_AXIS3 = numpy.array([1, 3, 5, 7, 9, 11], dtype=int)
...
def _run():
    ...
    for i in range(len(GRID_ROW_COUNTS_AXIS1)):
        for k in range(len(LAG_TIME_COUNTS_AXIS3)):
            ...
            output_file_name = (
                '{0:s}/num-grid-rows={1:03d}_num-lag-times={2:02d}/model.keras'
            ).format(
                OUTPUT_DIR_NAME,
